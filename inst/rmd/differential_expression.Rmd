...
if (!is.null(rmd_params$count_data)) {
  ...
  count_data <- readr::read_csv(
    rmd_params$path_salmon_counts,
    progress = FALSE
  )
}

# Same for normalized counts
if (!is.null(rmd_params$count_data_normalized)) {
  message("Using count_data_normalized dataframe provided as paramter")
  count_data_normalized <- rmd_params$count_data_normalized
} else {
  message("Loading counts from Salmon count CSV file")
  count_data_normalized <- readr::read_csv(
    rmd_params$path_salmon_tpm,
    progress = FALSE
  )
}
