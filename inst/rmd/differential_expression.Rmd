...
entries_satisfying_mincount <- count_data_normalized %>%
  ...
  dplyr::summarise(mean_count_normalized = mean(count_normalized)) %>%
...
We work with `r length(entries_satisfying_mincount)` genes where at least one treatment has a mean gene normalized count (in Salmon counts `TPM`) expression of `r rmd_params$minimum_count`.
...
if (rmd_params$biomart_attributes != "none") {
  paste0(
    "Attaching biomart variables ",
    rmd_params$biomart_attributes
  ) %>%
    message()

  deseq_result_frame %<>%
    rmyknife::attach_biomart(
      ensembl_version = rmd_params$biomart_version,
      ensembl_id_var = "row",
...
out_path_de_files <- file.path(rmd_params$out_path, "deseq_diff")

if (rmd_params$out_path != "") {
  glue::glue("You set the output file path to {out_path_de_files}") %>%  message()
  # Create the folder
  dir.create(out_path_de_files)
  # Save csv
  if (rmd_params$save_csv) {
    # Get proper path
    csv_path <- file.path(out_path_de_files, "deseq2_diff.csv")
  ...
  if (rmd_params$save_excel) {
    # Get proper path
    excel_path <- file.path(out_path_de_files, "deseq2_diff.xlsx")
    # Print status message
  ...
  # Save RDS
  if (rmd_params$save_deseq_rds) {
    # deseq2_diff
    deseq2_diff_path <- file.path(out_path_de_files, "deseq2_diff.rds")
    glue::glue("Save deseq2_diff file to {deseq2_diff_path}") %>% message()
    saveRDS(object = deseq_result, file = deseq2_diff_path)
    # col_data
    col_data_path <- file.path(out_path_de_files, "col_data.rds")
    glue::glue("Save col_data file to {col_data_path}") %>% message()
    saveRDS(object = deseq_result, file = col_data_path)
    # count_data
    count_data_path <- file.path(out_path_de_files, "count_data.csv")
    glue::glue("Save count_data file to {count_data_path}") %>% message()
    readr::write_csv(x = deseq_result, file = count_data_path)
    # count_data_normalized
    count_data_normalized_path <- file.path(out_path_de_files, "count_data_normalized.csv")
    glue::glue("Save count_data_normalized file to {count_data_normalized_path}") %>% message()
    readr::write_csv(x = deseq_result, file = count_data_normalized_path)
  }
}
```
...
```{r pca-with-labels, fig.width = 10, fig.height = 10}
...
| Column               | Description                                                                                                                                                                                               |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `row`                | [ensembl.org](https://www.ensembl.org/info/genome/stable_ids/index.html) gene stable ID                                                                                                                  |
