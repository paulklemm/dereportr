...
if (params$dev == TRUE) {
  ...
  rmarkdown_params$out_path <- "/beegfs/scratch/bruening_scratch/pklemm/2019-04-philipp-cers/nf-rnaseq/results/DESeq2"
  rmarkdown_params$save_csv <- TRUE
  rmarkdown_params$save_excel <- TRUE
  rmarkdown_params$save_deseq_rds <- TRUE
  rmarkdown_params$biomart_variables <- "external_gene_name"
  ...
  rmarkdown_params$biomart_variables <- params$biomart_variables
...
if (rmarkdown_params$out_path != "") {
  ...
  if (!dir.exists(rmarkdown_params$out_path)) {
    message("The output file path does not exist. I will create it now")
    dir.create(
      path = rmarkdown_params$out_path,
      showWarnings = TRUE,
      recursive = TRUE
    )
  }
