...
# Filter out genes where no samples contains more than minimum_gene_count
entries_satisfying_mincount <- count_data %>%
  tidyr::pivot_longer(-gene_id, values_to = "count") %>%
  dplyr::group_by(gene_id) %>%
  dplyr::summarise(max_count = max(count)) %>%
  dplyr::filter(max_count >= params$minimum_count) %>%
  dplyr::select(gene_id) %>%
  dplyr::pull()

count_data <- count_data %>%
  dplyr::filter(gene_id %in% entries_satisfying_mincount)
```

We work with `r length(entries_satisfying_mincount)` genes where at least one sample has a minimum count of `r params$minimum_count`.
...
In essence, it captures the same information as the PCA, but plotted in a different way.
...
| Column               | Description                                                                                                                                                                                               |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `row`                | (https://www.ensembl.org/info/genome/stable_ids/index.html)[ensembl.org gene stable ID]                                                                                                                   |
| `baseMean`           | (https://doi.org/10.1186/s13059-014-0550-8)[DESeq2] - average of the normalized count values, dividing by size factors (adjusting for different library size), taken over all samples.                    |
| `log2FoldChange`     | (https://doi.org/10.1186/s13059-014-0550-8)[DESeq2] - effect size estimate between treatment and control. A log2 fold change of 1.5 means the gene expression is increased by 2^1.5 = 2.82.               |
| `lfcSE`              | (https://doi.org/10.1186/s13059-014-0550-8)[DESeq2] - Uncertainty associated with `log2FoldChange`, representing the standard error.                                                                      |
| `stat`               | (https://doi.org/10.1186/s13059-014-0550-8)[DESeq2] - `log2FoldChange`/`lfcSE`. This is used to calculate the `pvalue`.                                                                                   |
| `pvalue`             | (https://doi.org/10.1186/s13059-014-0550-8)[DESeq2] - Probability that a fold change as strong as the observed one, or even stronger, would be seen under the situation described by the null hypothesis. |
| `padj`               | (https://doi.org/10.1186/s13059-014-0550-8)[DESeq2] - Benjamini-Hochberg adjusted `pvalue`, adjustment for false discovery rate (FDR).                                                                    |
| `comparison`         | Pairwise comparison for the current DESeq2 output.                                                                                                                                                        |
| `external_gene_name` | (https://www.ensembl.org/info/data/biomart/index.html)[Ensembl Biomart] gene name.                                                                                                                        |
| `gene_biotype`       | (https://www.ensembl.org/info/data/biomart/index.html)[Ensembl Biomart] (https://www.gencodegenes.org/pages/biotypes.html)[gene biotype].                                                                 |
| `description`        | (https://www.ensembl.org/info/data/biomart/index.html)[Ensembl Biomart] gene description.                                                                                                                 |
...
```{r volcano_plot, fig.width = 12, fig.height = 6, results = "asis"}
